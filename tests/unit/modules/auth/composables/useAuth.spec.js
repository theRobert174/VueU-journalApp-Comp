...
describe('Pruebas en useAuth', () => {
    ...
    test('createUser exitoso', async() => {
        ...
        const newUser = { name: 'Rodrigo', email: 'rodrigo@mail.com'}
        mockStore.dispatch.mockReturnValue({ok: true})

        const resp = await createUser(newUser)

        expect(mockStore.dispatch).toHaveBeenCalledWith('auth/createUser',{ 'name': 'Rodrigo', 'email': 'rodrigo@mail.com'})
    ...
    test('login exitoso', async() => {
        ...
        const loginForm = { email: 'rodrigo@mail.com', password: '123456'}
        mockStore.dispatch.mockReturnValue({ok: true})
